...
mkdir -p ~/tmp && cd "$_"
git clone -b v4.16.0 git@github.com:platformio/platform-teensy.git
guix shell platformio
pio pkg install -p "https://github.com/platformio/platform-teensy.git"
cd ~/tmp/platform-teensy/examples/zephyr-synchronization
